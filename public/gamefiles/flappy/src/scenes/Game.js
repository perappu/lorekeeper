...
*
*/
import ASSETS from '/gamefiles/flappy/src/assets.js';
import ANIMATION from '/gamefiles/flappy/src/animation.js';
...
export class Game extends Phaser.Scene {
    constructor() {
        ...
        //to prevent the user from closing the game after it's started
    ...
    create() {
        ...
        this.tutorialText = this.add.text(this.centreX, this.centreY, start_text, {
        ...
        this.physics.pause();
        this.tutorialText.setInteractive().once('pointerdown', () => {
            this.startGame();
        });
    ...
    initAnimations() {
        this.anims.create({
            key: ANIMATION.player.key,
            frames: this.anims.generateFrameNumbers(ANIMATION.player.texture),
            frameRate: ANIMATION.player.frameRate,
            repeat: ANIMATION.player.repeat
        });
        this.anims.create({
    ...
    initPlayer() {
        this.player = this.physics.add.sprite(200, this.centreY, ASSETS.spritesheet.player.key)
            .setDepth(100)
            .setCollideWorldBounds(true);
        this.player.anims.play(ANIMATION.player.key, true);
    }
    ...
    GameOver() {
        this.time.delayedCall(2000, () => {
            this.scene.start('GameOver', {'score' : this.score, 'beforeUnloadHandler' : this.beforeUnloadHandler});
        });
    }
}
